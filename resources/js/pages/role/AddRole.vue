...
const {permissions_by_group_name,total_permissions,errors,loading,storeLoading} = storeToRefs(pinia_role);
...
function implementALlChecked(){
  ...
  const storeRole = async() => {
    const resData = await pinia_role.store(form);
    if(resData?.status){
          ...
          <div class="card card-primary">
            ...
            <form @submit.prevent="storeRole">
                        ...
                        <div class="row" v-for="(group_permissions, group) in permissions_by_group_name" >
                          <div class="col-4">
                            <div class="icheck-primary">
                              <input type="checkbox" class="form-check-input" :value="group" :id="group+'Management'" @click="checkPermissionByGroup(`role-${group}-management-checkbox`,`${group}Management`,group)" >
                ...
                <div class="card-footer">
                  <button type="submit" class="btn btn-primary" :disabled="storeLoading">Submit</button>
